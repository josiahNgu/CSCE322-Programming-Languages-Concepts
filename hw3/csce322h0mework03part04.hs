...
manyPlayersOneMove di de ha= (manyMoves ha de di 0)

manyMoves:: [[(Char,Char)]]->[(Char,Char)]->[(Char,Char)]  ->Int -> ([[(Char,Char)]],[(Char,Char)],[(Char,Char)])
manyMoves hands de di index
          | (endGame hands) ==1 = (hands,de,[('-','-')]++di)
          | length di > 2 && tailDifst == 'w' && tailDisnd =='d' && haveWild4 ha /=1 = manyMoves ((insertAt index (drawDeck ha de ((countWDs di)*4 )) hands)) (removeDeck de ((countWDs di)*4 )) (addToDiscard ha di ha 1) (nextIndex (addToDiscard ha di ha 1) index (length hands))
          | length di > 2 && tailDifst == 'w' && tailDisnd =='d' && haveWild4 ha ==1 = manyMoves (((insertAt index (wild4 ha) hands))) (de) (addToDiscard ha di ha 5) (nextIndex (addToDiscard ha di ha 5)index (length hands))
          | fst(head di ) /='w' && snd(head di) =='d' && haveColorDraw ha ==1 = manyMoves ((insertAt index (draw2 ha) hands)) (de) (addToDiscard ha di ha 2) (nextIndex (addToDiscard ha di ha 2) index (length hands))
          | fst(head di) /='w' && snd(head di) =='d' && haveColorDraw ha /=1 = manyMoves ((insertAt index (drawDeck ha de((countCDs di)*2)) hands )) (removeDeck de ((countCDs di)*2)) (addToDiscard ha di ha 3)  (nextIndex (addToDiscard ha di ha 3) index (length hands))
          | haveColor ha di == 1 = manyMoves ((insertAt index (color ha di) hands)) (de) (addToDiscard ha di ha 4) (nextIndex (addToDiscard ha di ha 4) index (length hands))
          | haveWild4 ha == 1 = manyMoves ((insertAt index (wild4 ha) hands)) (de) (addToDiscard ha di ha 5)  (nextIndex (addToDiscard ha di ha 5) index (length hands))
          | haveSymbol ha di == 1 = manyMoves ((insertAt index (symbol ha di) hands)) (de) (addToDiscard ha di ha 6)  (nextIndex (addToDiscard ha di ha 6) index (length hands))
          | haveWild ha == 1 = manyMoves ((insertAt index (wild ha) hands)) (de) (addToDiscard ha di ha 7)  (nextIndex (addToDiscard ha di ha 7) index (length hands))
          | (length de) >0 =  manyMoves ((insertAt index (drawDeck ha de (1)) hands)) (removeDeck (de) 1) (di) (nextIndex di index (length hands))
